...
exports.getClubDetails = (req, res) => {
    ...
    if(schema_result.error){
        ...
        clubmaster.getClubDetails(req, res).then(data => {
            if (data.count > 0) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
...
exports.createClub = (req, res) => {
    const data = req.body;
    const schema = Joi.object().keys({
        clubName: Joi.string().required().error(new Error('Provide clubName(string)')),
        clubEntryPoint: Joi.number().required().error(new Error('Provide clubEntryPoint(number)'))
    });
    ...
    if(schema_result.error){
        ...
        clubmaster.createClub(req, res).then((data) => {
            if (data.dataValues) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
...
exports.updateClub = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        clubName: Joi.string().error(new Error('Provide clubName(string)')),
        clubEntryPoint: Joi.number().error(new Error('Provide clubEntryPoint(number)')),
