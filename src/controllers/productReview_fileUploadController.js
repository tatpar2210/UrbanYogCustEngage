...
module.exports = {
  getProductReview_fileUpload: async function (req, res) {
    ...
    const schema = Joi.object().keys({
      ...
      review_id: Joi.number()
        .optional()
        .allow("")
        .error(new Error("review_id must be a number")),
      file_id: Joi.number()
        .optional()
        .allow("")
        .error(new Error("file_id must be a number")),
      limit: Joi.number()
        .optional()
        .allow("")
    ...
    if (schema_result.error) {
      ...
      await product_review_file_uploadService
        .getFiles(req_data)
        .then((result) => {
          if (!result) {
            res.status(200).json({
  ...
  getProductReviewImages: async function (req, res) {
    ...
    product_review_file_uploadService
  ...
  postProductReview_fileUpload: async function (req, res) {
    ...
    const storage_loc = path.join(
      __dirname,
      "../../public/File_Uploads/Product-review/" + review_id + "/img"
    );
  ...
  postSingleRevImg: async function (req, res) {
    ...
    upload(req, res, function (err) {
      ...
      if (err) {
        ...
        product_review_file_uploadService
          .store_single_uploads(store_json_arr)
          .then((result) => {
            res.status(200).json({
              statusCode: 100,
              status: true,
