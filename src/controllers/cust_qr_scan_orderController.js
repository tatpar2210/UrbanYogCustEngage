...
module.exports = {
  async getAll_cust_qrscanned_order(req, res) {
    ...
    const schema = Joi.object().keys({
      call_req_id: Joi.number()
        .optional()
        .error(new Error("Provide call_req_id(number)")),
      cust_name: Joi.string()
        .optional()
        .allow("")
        .error(new Error("Provide cust_name(string)")),
      cust_email: Joi.string()
        .email()
        .optional()
        .allow("")
        .error(new Error("Provide cust_email(string | email)")),
      cust_number: Joi.number()
        .optional()
        .error(new Error("Provide cust_number(number)")),
      bought_product_pid: Joi.number()
        .optional()
        .allow("")
        .error(new Error("Provide bought_product_pid(number)")),
      bought_product: Joi.string()
        .optional()
        .allow("")
        .error(new Error("Provide bought_product(string)")),
      bought_from: Joi.string()
        .optional()
        .allow("")
        .error(new Error("Provide bought_from(string)")),
    ...
    if (schema_result.error) {
      ...
      await cust_qr_scan_order_service
        .getAll_cust_qr_scan_order(req_data)
        .then((data) => {
          if (data.count > 0) {
            res.status(200).send({
              statusCode: 100,
  ...
  async getCount_cust_qrscanned_order(req, res) {
    ...
    if (schema_result.error) {
      ...
      await cust_qr_scan_order_service
        .getCount_cust_qr_scan_order(req_data)
        .then((data) => {
          if (data > 0) {
            res.status(200).send({
              statusCode: 100,
  ...
  async create_cust_qr_scanned_order(req, res) {
    ...
    if (schema_result.error) {
      ...
      await cust_qr_scan_order_service
        .store_cust_qr_scan_order(req_data)
        .then((result) => {
          res.status(200).json({
            statuscode: 200,
