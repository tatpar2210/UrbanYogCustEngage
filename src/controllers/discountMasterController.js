...
exports.getDiscountInfo = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        discountTitle: Joi.string().error(new Error('Provide discountTitle(string)')),
    ...
    if(schema_result.error){
        ...
        discountservice.getDiscountInfo(req, res).then(data => {
            if (data.count > 0) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
    ...
    exports.getDiscountDetails = (req, res) => {
        ...
        if(schema_result.error){
            ...
            discountservice.getDiscountDetails(req, res).then(data => {
                if (data.count > 0) {
                    res.status(200).send({
                        statusCode: 100,
                        status: true,
    ...
    exports.createDiscountCode = (req, res) => {
        const data = req.body;
        const schema = Joi.object().keys({
            discountCode: Joi.string().required().error(new Error('Provide discountCode(number)')),
            discountTitle: Joi.string().required().error(new Error('Provide discountTitle(string)')),
            discountType: Joi.string().required().error(new Error('Provide discountType(string)')),
            discountValue: Joi.number().required().required().error(new Error('Provide discountValue(number)')),
            minimumRequirement: Joi.string().error(new Error('Provide minimumRequirement(string)')),
            minimumQuantity: Joi.number().error(new Error('Provide minimumQuantity(string)')),
            minimumAmount: Joi.number().error(new Error('Provide minimumAmount(string)')),
            usageLimit: Joi.number().error(new Error('Provide usagelimit(number)')),
            startDate: Joi.string().required().error(new Error('Provide startDate(string)')),
...
exports.updateDiscountCode = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        discountTitle: Joi.string().error(new Error('Provide discountTitle(string)')),
        discountType: Joi.string().error(new Error('Provide discountType(string)')),
        discountValue: Joi.number().error(new Error('Provide discountValue(number)')),
        minimumRequirement: Joi.string().error(new Error('Provide minimumRequirement(string)')),
        minimumQuantity: Joi.number().error(new Error('Provide minimumQuantity(string)')),
        minimumAmount: Joi.number().error(new Error('Provide minimumAmount(string)')),
        usageLimit: Joi.number().error(new Error('Provide usagelimit(number)')),
    ...
    if(schema_result.error){
        ...
        discountservice.updateDiscountCode(req, res).then(data => {
            console.log(data);
            if (data[0] > 0) {
                res.status(200).send({
                    statusCode: 100,
...
exports.deleteDiscountCode = (req, res) => {
    ...
    if(schema_result.error){
        ...
        discountservice.deleteDiscountCode(req, res).then(data => {
            if (data > 0) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
