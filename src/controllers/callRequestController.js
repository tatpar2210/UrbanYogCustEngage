...
exports.getCallRequest = (req, res) => {
    ...
    if(schema_result.error){
        ...
        callrequest.getCallRequest(req, res).then(data => {
            if (data.count > 0) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
...
exports.createCallRequest = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        contactNo: Joi.string().min(10).max(13).required().error(new Error('Provide contactNo(number) OR Please Check Lenght Of Number')),
    ...
    if(schema_result.error){
        ...
        callrequest.createCallRequest(req, res).then((data) => {
            if (data.dataValues) {
              res.status(200).send({
                statusCode: 100,
                status: true,
  ...
  exports.updateCallRequest = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        callReqId: Joi.number().required().error(new Error('Provide callReqId(number)')),
        custName: Joi.string().error(new Error('Provide custName(string)')),
        custEmail: Joi.string().error(new Error('Provide custEmail(string)')),
        status: Joi.number().error(new Error('Provide status(number)')),
        contactNo: Joi.string().min(10).max(13).error(new Error('Provide contactNo(number) OR Please Check Lenght Of Number')),
