...
module.exports = {
  getAllReviews: async function (req, res) {
    ...
    const schema = Joi.object().keys({
      ...
      star_count: Joi.number()
        ...
        .allow("")
        .max(5)
        .error(new Error("Provide star_count(number(upto value 5 only))")),
      custName: Joi.string()
        .optional()
        .allow("")
      ...
      review: Joi.string()
        .optional()
        .allow("")
        .error(new Error("Provide review(string)")),
      reviewTitle: Joi.string()
        .optional()
        .allow("")
        .error(new Error("Provide reviewTitle(string)")),
      status: Joi.number()
        .optional()
        .allow("")
      ...
      img: Joi.boolean()
        .optional()
        .allow("")
        .error(new Error("Set img as true to show reviews having images")),
      vid: Joi.boolean()
        .optional()
        .allow("")
        .error(new Error("Set vid as true to show reviews having videos")),
    });
    ...
    if (Object.keys(req_data).length === 0) {
      ...
      const schema = Joi.object().keys({
        ...
        sortBy: Joi.string()
          .optional()
          .allow("")
          .error(new Error("Provide sortBy(string)")),
        img: Joi.boolean()
          .optional()
          .allow("")
          .error(new Error("Set img as true to show reviews having images")),
    ...
    if (Object.keys(data).length === 0) {
      ...
      const schema = Joi.object().keys({
        ...
        cust_email: Joi.string()
          ...
          .lowercase()
          .error(new Error("Provide custEmail(string)")),
        cust_location: Joi.string()
          .optional()
          .allow("")
          .error(new Error("Provide custlocation(string)")),
        shopify_cust_id: null,
        status: Joi.number().error(new Error("Provide status(number)")),
        admin_reply: Joi.string()
          .optional()
          .allow("")
          .error(new Error("Provide admin_reply(string)")),
  ...
  getReviewStarCount: async function (req, res) {
    ...
    for (var i = 1; i < 6; i++) {
      await product_reviewService
        .getProductReviews_star_count(P_id, i)
        .then((result) => {
