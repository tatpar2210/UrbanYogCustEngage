...
exports.getSupportDetails = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        storeOrderNumber: Joi.number().error(new Error("Provide orderNumber(number)")),
            ...
            .then((data) => {
                ...
                if (data.dataValues) {
                    res.status(200).send({
                        statusCode: 100,
                        status: true,
                        message: "Support request raised, our customer executive will contact you shortly",
                        data: data,
                    });
...
exports.updateSupportDetails = (req, res) => {
    const data = req.body;
    const schema = Joi.object().keys({
        supportId: Joi.number().required().error(new Error("Provide supportId(number)")),
