...
exports.addCustAddress = (req, res) => {
    ...
    const schema = Joi.object().keys({
        shopifyCustomerId: Joi.number().required().error(new Error("Provide shopifyCustomerId(number)")),
        address1: Joi.string().required().error(new Error("Provide address1(string)")),
        address2: Joi.string().required().error(new Error("Provide address2(string)")),
        city: Joi.string().required().error(new Error("Provide city(string)")),
        landMark: Joi.string().required().error(new Error("Provide landMark(string)")),
        ...
        province: Joi.string().required().error(new Error("Provide province(string)")),
        country: Joi.string().required().error(new Error("Provide country(string)")),
        zip: Joi.string().required().error(new Error("Provide zip(string)")),
        countryName: Joi.string().required().error(new Error("Provide countryName(string)"))
    });
        ...
        customerHelper
            ...
            .then((data) => {
                if (data == "Not Found") {
                    ...
                    customerAddressService.addCustAddress(data.customer_address,  res)
                        .then((data) => {
                            if (data) {
                                res.status(200).send({
                                    statusCode: 100,
...
exports.deletCustAddress = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        shopifyAddressId: Joi.string().required().error(new Error("Provide shopifyAddressId(string)"))
    });
    ...
    if(schema_result.error){
        ...
        customerHelper.deleteCustAddress(req.body, res)
            .then((shopfiyRes) => {
                ...
                if (shopfiyRes.errors) {
                    ...
                    customerAddressService.deleteCustAddress(req.body, res)
                        .then((data) => {
                            if (data > 0) {
                                res.status(200).send({
                                    statusCode: 100,
