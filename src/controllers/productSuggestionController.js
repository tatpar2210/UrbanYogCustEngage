...
module.exports = {
    ...
    createProductSuggestionDetails: async function(req, res){
        ...
        const schema = Joi.object().keys({
            pId: Joi.number().error(new Error('Provide pId(number)')),
            suggstPid: Joi.array().items(
                Joi.object({
                suggst_pid: Joi.number().required().error(new Error('Provide suggst_pid(number)'))
                })
                ).required(),
    ...
    deleteProductSuggestionDetails: async function(req, res){
        ...
        if(schema_result.error){
            ...
            product_suggestion_master.deleteProductSuggestionDetails(req, res).then(data => {                
                if (data > 0) {
                    res.status(200).send({
                        statusCode: 100,
                        status: true,
