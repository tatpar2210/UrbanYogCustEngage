...
exports.getTPMReview = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        tpmProductUrl: Joi.string().error(new Error('Provide tpmProductUrl(number)')),
    ...
    if (schemaresult.error){
        ...
        tpmreview.getTPMReview(req, res).then(data => {
            if (data.count > 0) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
...
exports.createTPMReview = (req, res) => {
    ...
    if(schemaresult.error){
        ...
        tpmreview.createTPMReview(req, res).then((data) => {
            if (data.dataValues) {
              res.status(200).send({
                statusCode: 100,
                status: true,
  ...
  exports.updateTPMReview = (req, res) => {
    ...
    const schema = Joi.object().keys({
        ...
        tpReviewId: Joi.number().required().error(new Error('Provide tpReviewId(number)')),
        tpId: Joi.number().error(new Error('Provide tpId(number)')),
        pId: Joi.number().error(new Error('Provide pId(number)')),
        tpmProductUrl: Joi.string().error(new Error('Provide tpmProductUrl(number)')),
