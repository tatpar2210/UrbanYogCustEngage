...
exports.convertRewardToDiscount = (req, res) => {
  const data = req.body;
  const schema = Joi.object().keys({
    custStoreId: Joi.number()
      .required()
      .error(new Error("Provide custStoreId(number)")),
    rewardPoint: Joi.number()
      .min(1)
      .max(100)
      .required()
      .error(
        new Error(
          "Provide rewardPoint(number) and should be greater then 1 and less than 100"
        )
      ),
  });
  ...
  if (schema_result.error) {
    ...
    custWalletOrder
      .convertRewardToDiscount(req, res)
      .then((data) => {
