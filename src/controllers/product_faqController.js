...
module.exports = {
    ...
    createProductFAQs: async function(req, res){
        ...
        const schema = Joi.object().keys({
            ...
            question: Joi.string().required().error(new Error("Provide question(string)")),
            answer: Joi.string().required().error(new Error("Provide answer(string)")),
        });
        ...
        if(schema_result.error){
            ...
            product_faq_service.createProductFAQs(req, res).then((data) => {
                if (data.dataValues) {
                  res.status(200).send({
                    statusCode: 100,
                    status: true,
