...
exports.getMetaKeyword = (req, res) => {
    ...
    if(schema_result.error){
        ...
        seoMetaKeyword.getMetaKeyword(req, res).then(data => {
            if (data.count > 0) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
...
exports.createMetaKeyword = (req, res) => {
    const data = req.body;
    const schema = Joi.object().keys({
        keyword: Joi.string().required().error(new Error('Provide keyword(string)')),
    ...
    if(schema_result.error){
        ...
        seoMetaKeyword.createMetaKeyword(req, res).then((data) => {
            if (data.dataValues) {
                res.status(200).send({
                    statusCode: 100,
                    status: true,
...
exports.updateMetaKeyword = (req, res) => {
    const data = req.body;
    const schema = Joi.object().keys({
        keywordId: Joi.number().required().error(new Error('Provide keywordId(number)')),
        keyword: Joi.string().error(new Error('Provide keyword(string)')),
        pId: Joi.number().error(new Error('Provide pid(number)')),
    ...
    if(schema_result.error){
        ...
        seoMetaKeyword.updateMetaKeyword(req, res)
            .then((data) => {
                if (data[0] > 0) {
                    res.status(200).send({
                        statusCode: 100,
...
exports.deleteMetaKeyword = (req, res) => {
    ...
    if(schema_result.error){
        ...
        seoMetaKeyword.deleteMetaKeyword(req, res)
            .then((data) => {
                if (data > 0) {
                    res.status(200).send({
                        statusCode: 100,
