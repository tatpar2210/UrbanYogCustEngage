...
class CustomerWalletService {
    ...
    getWalletDetails(req, res) {
        return new Promise((resolve, reject) => {
            ...
            CustomerWallet.belongsTo(CustomerMaster, { foreignKey: 'store_cust_id', targetKey: 'shopify_customer_id' })
            CustomerWallet.belongsTo(ClubMaster, { foreignKey: 'club_id', targetKey: 'club_id' })

            return CustomerWallet.findAndCountAll({
                where: where,
                attributes: ['wallet_id', 'total_amount', 'status', 'created_at', 'updated_at'],
                ...
                include: [
                    {
                        ...
                        where: custWhere,
                        attributes: ['cust_id', 'shopify_customer_id', 'first_name', 'last_name', 'email', 'phone']
                    },
                    {
                        model: ClubMaster,
    ...
    getWalletCustomerDetails(req, res) {
        return new Promise((resolve, reject) => {
            ...
            CustomerMaster.belongsTo(CustomerWallet, { foreignKey: 'shopify_customer_id', targetKey: 'store_cust_id' })

            return CustomerMaster.findAndCountAll({
                where: whereCustomer,
                offset: req.body.offset || 0,
                limit: req.body.limit || 50,
                attributes: [[Sequelize.literal("CONCAT(first_name, ' ', last_name)"), 'full_name'], 'shopify_customer_id', 'first_name', 'last_name', 'email', 'phone'],
