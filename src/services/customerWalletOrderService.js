...
class customerWalletOrderService {
    async addReward(webhookData, res) {
        ...
        try {
            ...
            if (walletData.statusCode = 101) {
                ...
                orderData.pid = null;
                orderData.tpmId = 3;
                orderData.transcationNote = 'Reward point is credited aginst order number ' + storeOrderNumber;
                ...
                if (walletHistory.statusCode == 100) {
                    ...
                    findWalletHistory.storeCustId = walletHistory.data.dataValues.store_cust_id;
                    ...
                    let creditArr = walletHistoryData.data.rows.filter((item) => {
                        if (item.dataValues.transaction_type == "credit" && item.dataValues.status == 1) {
                            return item;
                        }
                    })

                    let debitArr = walletHistoryData.data.rows.filter((item) => {
                        if (item.dataValues.transaction_type == "debit" && item.dataValues.transaction_status == 2 && item.dataValues.status == 0) {
                            return item;
                        }
                    })

                    let discountArr = walletHistoryData.data.rows.filter((item) => {
                        if (item.dataValues.transaction_type == "debit" && item.dataValues.transaction_status == 3 && item.dataValues.status == 0) {
                            return item;
                        }
                    })
                    ...
                    let activeCreditArr = walletHistoryData.data.rows.filter(value => {
                        var count = 0;
                        for (var i = 0; i < len; i++) {
                            if (walletHistoryData.data.rows[i].store_order_id === value.store_order_id)
                                count++;
                        }
                        return count === 1;
                    })
                    ...
                    let unusedActiveCreditArr = activeCreditArr.filter((item) => {
                        if (item.dataValues.transaction_type == "credit" && item.dataValues.transaction_status == 1) {
                            return item;
                        }
                    })
        ...
        try {
            ...
            if (walletHistoryData.statusCode == 100) {
                if (walletHistoryData.data.count == 1) {
                    ...
                    orderData.transcationNote = 'Reward point is debited aginst cancellation of order number ' + storeOrderNumber;
                    ...
                    let walletHistory = await this.createWalletHistory(orderData);
                    let updateWalletHistoryData = await this.updateWalletHistoryStatus(orderData)
                    if (walletHistory.statusCode == 100) {
                        let findWalletHistory = {};
                        walletId = walletHistory.data.dataValues.wallet_id
                        findWalletHistory.walletId = walletHistory.data.dataValues.wallet_id;
        ...
        if (webhookFulfillmentEvent.shipment_status == "delivered") {
            ...
            try {
                ...
                if (walletHistory.statusCode == 100) {
                    ...
                    findWalletHistory.walletId = walletHistory.data.rows[0].dataValues.wallet_id;
                    findWalletHistory.storeCustId = walletHistory.data.rows[0].dataValues.store_cust_id;
                    ...
                    orderData.status = 1;
                    let updateWalletHistoryData = await this.updateWalletHistoryTransactionStatus(orderData)
        ...
        try {
            ...
            if (custWallet.statusCode == 100) {
                if (discountPoint <= custWallet.data.dataValues.total_amount) {
                    ...
                    let priceruleObj = await discountHelper.createPriceRule(priceruleData);
                    if (priceruleObj && priceruleObj.price_rule) {
                        let { price_rule } = priceruleObj;
                        discountTitle = price_rule.title;
                        let discountObj = await discountHelper.createDiscount(price_rule.title, price_rule.id)
                        ...
                        discountData.transcationNote = 'Reward point converted to discount code ' + discountTitle + ' worth Rs. ' + discountPoint;
                        ...
                        let updateWalletHistoryData = await this.createWalletDiscountHistory(discountData)
                    ...
                    errResp = {
                        "statusCode": 101,
                        "status": false,
                        "message": "Reward point should be less than active reward point in wallet",
                        "data": []
                    }
                    return errResp;
                }
    ...
    getCode() {
        return new Promise((resolve, reject) => {
            db.sequelize.query(`SELECT LEFT(REPLACE(UUID(), '-', ''), 5) as code`,
                { type: QueryTypes.SELECT }).then((code) => {
                    resolve(code);
    ...
    createWallet(storeCustId, amount) {
        ...
        return new Promise((resolve, reject) => {
            ...
            return CustomerWallet.findAll(cnd).then((result) => {
                if (result.length <= 0) {
                    let clubData = {};
                    let clubId = 1;
                    clubData.clubId = clubId;
                    return this.getClub(clubData).then(clubData => {
                        let percentage = clubData.data.dataValues.discount_percentage;
                ...
                if (result.length <= 0) {
                    return CustomerWalletHistory.create({
                        ...
                        let errResp = {
                            statusCode: 100,
                            status: true,
                            message: 'Reward points added against order number ' + orderData.storeOrderNumber,
                            data: rewardCreditdata
                        }
                        resolve(errResp);
                    })
    ...
    getAllClub() {
        let errResp = {};
        return new Promise((resolve, reject) => {
            return ClubMaster.findAll({}).then(custData => {
                if (custData) {
                    errResp.statusCode = 100;
                    errResp.message = 'Club details found';
                    errResp.data = custData;
